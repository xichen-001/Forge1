...
class CharEnv(sim_env.SimEnv):
    def __init__(self, env_config, engine_config, num_envs, device, visualize):
        ...
        self._enable_dof_pos_obs = env_config.get("enable_dof_pos_obs", False)
        self._enable_last_action_obs = env_config.get("enable_last_action_obs", False)
        self._action_dof_names = env_config.get("action_dof_names", None)
        self._action_dof_indices = None
    ...
    def _build_envs(self, env_config, num_envs):
        ...
        self._init_action_dof_subset(env_config)
    ...
    def _get_dof_names(self):
        ...
        for j in range(1, num_joints):
            ...
            if (dof_dim == 1):
                ...
                dof_names.extend([f"{joint.name}_x", f"{joint.name}_y", f"{joint.name}_z"])
        return dof_names

    def _init_action_dof_subset(self, env_config):
        if not self._action_dof_names:
            return
        dof_names = self._get_dof_names()
        name_to_idx = {name: idx for idx, name in enumerate(dof_names)}
        missing = [name for name in self._action_dof_names if name not in name_to_idx]
        if missing:
            raise ValueError(f"Unknown action_dof_names: {missing}")
        indices = [name_to_idx[name] for name in self._action_dof_names]
        self._action_dof_indices = torch.tensor(indices, device=self._device, dtype=torch.long)
        return
    ...
    def _build_action_space(self):
        ...
        if (self._action_dof_indices is not None):
            idx = self._action_dof_indices.cpu().numpy()
            low = low[idx]
            high = high[idx]
    ...
    def _append_extra_obs(self, obs, dof_pos, env_ids=None):
        if (self._enable_dof_pos_obs):
            dof_pos_obs = dof_pos
            if (self._action_dof_indices is not None):
                dof_pos_obs = dof_pos_obs[:, self._action_dof_indices]
            obs = torch.cat([obs, dof_pos_obs], dim=-1)

        if (self._enable_last_action_obs):
            if (not hasattr(self, "_last_action")):
                action_dim = int(self._action_space.shape[0])
                num_envs = self.get_num_envs()
                self._last_action = torch.zeros([num_envs, action_dim], device=self._device, dtype=torch.float32)
            last_action = self._last_action
            if (env_ids is not None):
                last_action = last_action[env_ids]
    ...
    def _apply_action(self, actions):
        if (self._action_dof_indices is not None):
            ...
            full_actions = dof_pos.clone()
            full_actions[:, self._action_dof_indices] = clip_action
            self._engine.set_cmd(char_id, full_actions)
            return
        char_id = self._get_char_id()
        clip_action = torch.minimum(torch.maximum(actions, self._action_bound_low), self._action_bound_high)
