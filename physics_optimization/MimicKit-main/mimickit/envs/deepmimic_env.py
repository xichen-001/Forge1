...
class DeepMimicEnv(char_env.CharEnv):
    #read config parameters from env_config
    def __init__(self, env_config, engine_config, num_envs, device, visualize):
        ...
        self._enable_privileged_obs = env_config.get("enable_privileged_obs", False)
        self._priv_root_vel_obs = env_config.get("priv_root_vel_obs", False)
        self._priv_dof_diff_obs = env_config.get("priv_dof_diff_obs", False)
    ...
    def _build_sim_tensors(self, env_config):
        ...
        foot_bodies = env_config.get("foot_bodies", [])
        self._foot_body_ids = self._build_body_ids_tensor(foot_bodies)
        self._foot_slip_w = env_config.get("foot_slip_w", 0.0)
        self._foot_contact_height = env_config.get("foot_contact_height",
                                                   env_config.get("contact_height", 0.06))
        self._foot_contact_vz = env_config.get("foot_contact_vz",
                                               env_config.get("contact_vz", 0.2))
        self._action_rate_w = env_config.get("action_rate_w", 0.0)
        self._torso_ang_vel_w = env_config.get("torso_ang_vel_w", 0.0)
        torso_body = env_config.get("torso_body", "torso_link")
        self._torso_body_ids = self._build_body_ids_tensor([torso_body])
        if (self._action_rate_w > 0.0):
            action_dim = int(self._action_space.shape[0])
            num_envs = self.get_num_envs()
            self._prev_action = torch.zeros([num_envs, action_dim], device=self._device, dtype=torch.float32)
    ...
    def _parse_joint_err_weights(self, joint_err_w):
        ...
        if (getattr(self, "_action_dof_indices", None) is not None):
            dof_mask = torch.zeros_like(self._dof_err_w)
            dof_mask[self._action_dof_indices] = 1.0
            for j in range(1, num_joints):
                dof_dim = self._kin_char_model.get_joint_dof_dim(j)
                if (dof_dim > 0):
                    dof_idx = self._kin_char_model.get_joint_dof_idx(j)
                    if (torch.sum(dof_mask[dof_idx:dof_idx + dof_dim]) == 0):
                        self._joint_err_w[j - 1] = 0.0
                        self._dof_err_w[dof_idx:dof_idx + dof_dim] = 0.0
        return
    ...
    def _has_foot_bodies(self):
        return (self._foot_body_ids is not None) and (self._foot_body_ids.numel() > 0)
    ...
    def _compute_obs(self, env_ids=None):
        ...
        obs = self._append_extra_obs(obs, dof_pos, env_ids)

        if (self._enable_privileged_obs):
            priv_obs = []
            if (self._priv_dof_diff_obs):
                ref_dof_pos = self._ref_dof_pos
                if (env_ids is not None):
                    ref_dof_pos = ref_dof_pos[env_ids]
                dof_diff = dof_pos - ref_dof_pos
                if (self._action_dof_indices is not None):
                    dof_diff = dof_diff[:, self._action_dof_indices]
                priv_obs.append(dof_diff)

            if (self._priv_root_vel_obs):
                ...
                if (not self._global_obs):
                    ...
                    ref_root_vel = torch_util.quat_rotate(heading_inv, ref_root_vel)
                priv_obs.append(ref_root_vel)

            if (len(priv_obs) > 0):
                obs = torch.cat([obs] + priv_obs, dim=-1)
        return obs
    ...
    def _update_reward(self):
        ...
        if (self._foot_slip_w > 0.0 and self._has_foot_bodies()):
            foot_pos = body_pos[..., self._foot_body_ids, :]
            foot_vel = body_vel[..., self._foot_body_ids, :]
            contact = (foot_pos[..., 2] < self._foot_contact_height) & (torch.abs(foot_vel[..., 2]) < self._foot_contact_vz)
            foot_vel_xy = torch.norm(foot_vel[..., :2], dim=-1)
            slip = torch.where(contact, foot_vel_xy, torch.zeros_like(foot_vel_xy))
            slip = torch.mean(slip, dim=-1)
            self._reward_buf[:] = self._reward_buf - self._foot_slip_w * slip

        if (self._action_rate_w > 0.0):
            action_rate = torch.norm(self._last_action - self._prev_action, dim=-1)
            self._reward_buf[:] = self._reward_buf - self._action_rate_w * action_rate

        if (self._torso_ang_vel_w > 0.0 and self._torso_body_ids.numel() > 0):
            torso_ang = body_ang_vel[..., self._torso_body_ids, :]
            torso_ang = torch.norm(torso_ang, dim=-1).mean(dim=-1)
            self._reward_buf[:] = self._reward_buf - self._torso_ang_vel_w * torso_ang
        if (self._action_rate_w > 0.0):
    ...
    def _fetch_tar_obs_data(self, motion_ids, motion_times):
        ...
        motion_ids_tiled = motion_ids.unsqueeze(-1).expand(motion_times.shape)
