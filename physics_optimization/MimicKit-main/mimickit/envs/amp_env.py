...
def _sanitize_quat(q):
    q = torch.nan_to_num(q, nan=0.0, posinf=0.0, neginf=0.0)
    norm = torch.linalg.norm(q, dim=-1, keepdim=True)
    q = q / norm.clamp_min(1e-6)
    invalid = (~torch.isfinite(norm)) | (norm < 1e-3)
    ident = torch.zeros_like(q)
    ident[..., 3] = 1.0
    q = torch.where(invalid, ident, q)
    return q
...
class AMPEnv(deepmimic_env.DeepMimicEnv):
    ...
    def _fetch_disc_demo_data(self, motion_ids, motion_times0):
        ...
        root_rot = _sanitize_quat(root_rot)
        joint_rot = _sanitize_quat(joint_rot)
    ...
    def _update_disc_obs(self, env_ids=None):
        ...
        # sanitize root rotations to avoid invalid quats on GPU
        root_rot = _sanitize_quat(root_rot)
        ...
        ref_root_rot = _sanitize_quat(ref_root_rot)
        joint_rot = _sanitize_quat(joint_rot)
