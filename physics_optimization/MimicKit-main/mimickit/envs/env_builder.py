...
def build_env(env_file, engine_file, num_envs, device, visualize):
    ...
    elif (env_name == "latent_amp"):
        import envs.latent_amp_env as latent_amp_env
        env = latent_amp_env.LatentAMPEnv(env_config=env_config, engine_config=engine_config, num_envs=num_envs, device=device, visualize=visualize)
    elif (env_name == "ase"):
