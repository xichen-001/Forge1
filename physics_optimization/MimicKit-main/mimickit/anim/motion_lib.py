...
class MotionLib():
    ...
    def _load_motion_pkl(self, motion_file):
        ...
        for f in range(num_motion_files):
            ...
            if curr_file.endswith(".npz"):
                curr_motion = self._load_motion_npz(curr_file)
            else:
    ...
    def _load_motion_npz(self, motion_file):
        ...
        root_rot_quat = torch.tensor(body_rot[:, pelvis_idx, :], dtype=torch.float32)
        # sanitize quaternion
        if torch.isnan(root_rot_quat).any() or torch.isinf(root_rot_quat).any():
            root_rot_quat = torch.zeros_like(root_rot_quat)
            root_rot_quat[:, 3] = 1.0
        else:
            root_rot_quat = torch_util.quat_normalize(root_rot_quat)

        root_rot = torch_util.quat_to_exp_map(root_rot_quat).cpu().numpy()

        frames = np.concatenate([root_pos, root_rot, dof_pos], axis=-1).astype(np.float32)
        return motion.Motion(loop_mode=motion.LoopMode.CLAMP, fps=fps, frames=frames)
