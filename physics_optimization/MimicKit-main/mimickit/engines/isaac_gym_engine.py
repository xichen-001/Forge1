...
class IsaacGymEngine(engine.Engine):
    def __init__(self, config, num_envs, device, visualize):
        ...
        self._enable_camera = config.get("enable_camera", False)
        self._camera_width = int(config.get("camera_width", 1280))
        self._camera_height = int(config.get("camera_height", 720))
        self._camera_handle = None
        self._camera_env = None
        self._camera_pos = np.array([0.0, -5.0, 3.0], dtype=np.float32)
        ...
        self._ground_static_friction = config.get("ground_static_friction", 1.0)
        self._ground_dynamic_friction = config.get("ground_dynamic_friction", 1.0)
        self._ground_restitution = config.get("ground_restitution", 0.0)
    ...
    def set_camera_pose(self, pos, look_at):
        if (self._viewer is not None):
            ...
            self._ensure_camera_sensor()
            self._gym.set_camera_location(self._camera_handle, self._camera_env,
                                          gymapi.Vec3(pos[0], pos[1], pos[2]),
                                          gymapi.Vec3(look_at[0], look_at[1], look_at[2]))
            self._camera_pos = np.array(pos, dtype=np.float32)
            self._camera_target = np.array(look_at, dtype=np.float32)
        return
    ...
    def get_camera_dir(self):
        if (self._viewer is not None):
            ...
            return cam_dir
        cam_delta = self._camera_target - self._camera_pos
        dist = np.linalg.norm(cam_delta)
        if dist < 1e-6:
            return np.array([0.0, 0.0, 1.0], dtype=np.float32)
        return (cam_delta / dist).astype(np.float32)
    ...
    def capture_rgb(self):
        ...
        img = self._gym.get_camera_image(self._sim, self._camera_env, self._camera_handle, gymapi.IMAGE_COLOR)
    ...
    def _build_ground(self):
        ...
        plane_params.static_friction = self._ground_static_friction
        plane_params.dynamic_friction = self._ground_dynamic_friction
        plane_params.restitution = self._ground_restitution
    ...
    def _create_simulator(self, sim_timestep, visualize):
        ...
        if (visualize or self._enable_camera):
    ...
    def _ensure_camera_sensor(self):
        if (self._camera_handle is None):
            if (len(self._envs) == 0):
                raise RuntimeError("No envs available for camera sensor")
            env_ptr = self._envs[0]
            cam_props = gymapi.CameraProperties()
            cam_props.width = self._camera_width
            cam_props.height = self._camera_height
            self._camera_handle = self._gym.create_camera_sensor(env_ptr, cam_props)
            self._camera_env = env_ptr
        return
