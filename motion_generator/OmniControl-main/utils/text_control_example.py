...
def combination_text_control_example(n_frames=120, raw_mean=None, raw_std=None, index=0):
    ...
    control = [
        ...
        [specify_points(n_frames, [[50, 0.18, 1.0, 0.8]]), specify_points(n_frames, [[50, -0.18, 1.0, 0.8]])],
    ]
    joint_id = np.array([
        [20, 21],
        [15, 21],
...
def straight_forward_step_uniform(
    n_frames=120, indices=[0, 2], scale=1.0, x_offset=0.5, y_offset=0.5, z_offset=0.5, step_ratio=0.5
):
    hint = np.zeros((n_frames, 3))
...
def straight_forward_backward_uniform(
    n_frames=120, indices=[0, 2], scale=1.0, x_offset=0.5, y_offset=0.5, z_offset=0.5
):
    hint = np.zeros((n_frames, 3))
...
def collate_all(n_frames, dataset):
    ...
    texts8, hints8, _ = text_to_motion_walk_wave_left(n_frames, raw_mean, raw_std)
    texts = texts0 + texts1 + texts2 + texts3 + texts4 + texts5 + texts6 + texts7 + texts8
    hints = np.concatenate([hints0, hints1, hints2, hints3, hints4, hints5, hints6, hints7, hints8], axis=0)
    return texts, hints


# def text_to_motion_walk_wave_kick(n_frames, raw_mean, raw_std):
#     import numpy as np

#     text = ["a person walks forward while waving with their left hand."]

#     pelvis_traj = straight_forward_step_uniform(
#         n_frames, indices=[0, 2], scale=4.0, x_offset=0.0, y_offset=0.9, z_offset=0.0, step_ratio=0.3
#     ).astype(np.float32)

#     l_wrist_traj = np.zeros((n_frames, 3), dtype=np.float32)
#     wave_full = circle(n_frames, r=0.08, indices=[0, 2], x_offset=-0.15, y_offset=0.6, z_offset=1.4)
#     start_wave, end_wave = 45, 76
#     l_wrist_traj[start_wave:end_wave, :] = wave_full[start_wave:end_wave]

#     control_full = np.zeros((1, n_frames, 22, 3), dtype=np.float32)
#     joint_ids = [0, 20, 10]

#     mean_rs = raw_mean.reshape(22, 1, 3)
#     std_rs = raw_std.reshape(22, 1, 3)

#     trajs = [pelvis_traj, l_wrist_traj]
#     for idx_in_list, joint in enumerate(joint_ids):
#         traj = trajs[idx_in_list]
#         mask = traj.sum(axis=-1) != 0
#         mean_j = mean_rs[joint]
#         std_j = std_rs[joint]
#         ctrl = (traj - mean_j) / (std_j + 1e-8)
#         ctrl = ctrl * mask[:, np.newaxis].astype(np.float32)
#         control_full[0, :, joint, :] = ctrl

#     control_flat = control_full.reshape((1, n_frames, -1))
#     return text, control_flat, np.array(joint_ids, dtype=np.int32)
...
#     text, control_flat, joint_ids = text_to_motion_walk_wave_kick(n_frames, raw_mean, raw_std)
#     print("text:", text)
#     print("control shape:", control_flat.shape)  # expect (1, n_frames, 66)
#     print("joint ids:", joint_ids)


def text_to_motion_walk_wave_left(n_frames, raw_mean, raw_std):
    """
    生成动作: "a person walks forward while waving with their left hand."
    返回:
      text: list of str
      control_flat: np.array of shape (1, n_frames, 66) dtype float32
      joint_ids: np.array([0, 20])  # pelvis, left_wrist
    """
    # 文本描述（与动作对应）
    text = ["a person walks forward while waving with their left hand."]

    # 1) pelvis: 向前走（连续前进）
    pelvis_traj = straight_forward_uniform(
        n_frames,
        indices=[0, 2],  # x,z 平面前进
        scale=4.0,
        x_offset=0.0,
        y_offset=0.9,
        z_offset=0.0,
    ...
    # 2) left wrist: 持续小圈作为挥手动作
    #    circle 生成的轨迹是围绕原点的，为了让挥手“随人走”，
    #    我们把它移动到 pelvis 的位置（做相对位移）
    l_wrist_traj = circle(n_frames, r=0.12, indices=[0, 2], x_offset=-0.15, y_offset=0.6, z_offset=1.3).astype(
        np.float32
    )

    # 让左手的圈跟随 pelvis 的平移（只加入 x 和 z 的相对移动）
    # 使用 pelvis_traj - pelvis_traj[0] 使得圈心随骨盆移动，但保持起始圈心的相对偏移
    pelvis_offset_x = pelvis_traj[:, 0] - pelvis_traj[0, 0]
    pelvis_offset_z = pelvis_traj[:, 2] - pelvis_traj[0, 2]
    l_wrist_traj[:, 0] += pelvis_offset_x
    l_wrist_traj[:, 2] += pelvis_offset_z

    # 3) 把轨迹写进大盒子 (batch=1, n_frames, 22, 3)
    control_full = np.zeros((1, n_frames, 22, 3), dtype=np.float32)
    joint_ids = [0, 20]  # pelvis, left_wrist

    # 4) 标准化并写入（与文件其它地方的做法一致）
    mean_rs = raw_mean.reshape(22, 1, 3)
    std_rs = raw_std.reshape(22, 1, 3)

    trajs = [pelvis_traj, l_wrist_traj]
    for idx_in_list, joint in enumerate(joint_ids):
        traj = trajs[idx_in_list]  # (n_frames, 3)
        mask = traj.sum(axis=-1) != 0  # 哪些帧有效（这里通常都是 True）

        mean_j = mean_rs[joint]  # (1,3)
        std_j = std_rs[joint]  # (1,3)
