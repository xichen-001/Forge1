...
def main():
    ...
    for rep_i in range(args.num_repetitions):
        ...
        sample = sample[:, :263]
        # Save MG latent before any conversion to xyz
        sample_latent = sample.detach().cpu()
        latent = sample_latent.permute(0, 3, 1, 2).reshape(sample_latent.shape[0], sample_latent.shape[3], -1)
    ...
    all_latents = np.concatenate(all_latents, axis=0)[:total_num_samples]
    if 'hint' in model_kwargs['y']:
    ...
    latent_path = os.path.join(out_path, 'latent.npz')
    print(f"saving latent file to [{latent_path}]")
    np.savez(latent_path, latent=all_latents, lengths=all_lengths, fps=fps, data_rep=model.data_rep)
